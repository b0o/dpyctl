...
# teensy_loader_cli path
# If you're using an old version of teensy_loader_cli, the binary is possibly
# named teensy-loader-cli (this is the case for the binaries installed via the
# `teensyduino` and `teensy-loader-cli` packages on Arch Linux)
# TEENSYLOADER = /usr/bin/teensy-loader-cli

# If you're using recent versions of teensy_loader_cli, the binary's name uses
# underscores:
TEENSYLOADER = /usr/bin/teensy_loader_cli
...
build: $(DISTDIR)/$(TARGET).elf

hex: $(DISTDIR)/$(TARGET).hex

# upload: $(TARGET).hex
upload: hex
	@$(TEENSYLOADER) -mmcu=mkl26z64 -w -v $(DISTDIR)/$(TARGET).hex
...
$(DISTDIR)/$(TARGET).elf: $(OBJS) $(LDSCRIPT)
	@echo -e "[LD]\t$@"
	@mkdir -p "$(dir $@)"
	@$(CC) $(LDFLAGS) -o "$@" $(OBJS) $(LIBS)

$(DISTDIR)/$(TARGET).hex: $(DISTDIR)/$(TARGET).elf
	@echo -e "[HEX]\t$@"
	@mkdir -p "$(dir $@)"
	@$(SIZE) "$<"
